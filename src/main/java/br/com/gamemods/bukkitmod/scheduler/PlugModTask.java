...
public class PlugModTask<R> implements FutureBukkitTask<R>
{
    ...
    public PlugModTask(PlugModScheduler scheduler, int taskId, Plugin plugin, boolean sync, Callable<R> callable, long delay, long timer)
    {
    ...
    public class PlugModWorker implements BukkitWorker
    {
        private Thread thread;

        private void start()
        {
            thread = new Thread(new AsyncWorker(), "PlugMod.AsyncTask."+taskId);
            thread.start();
        }
        ...
        private class AsyncWorker implements Runnable
        {
            ...
            public void run()
            {
                try
                {
                    scheduler.workers.add(PlugModWorker.this);
                    execute();
                }
                catch (Throwable e)
                {
                ...
                {
                    scheduler.workers.remove(PlugModWorker.this);
                    if(!isCancelled() && timer >= 0)
                        nextRun = timer;
                    else
                        scheduler.cancelTask(PlugModTask.this);
                }
            }
        }
        ...
        {
            return PlugModTask.this;
        }

        public PlugModTask<R> getPlugModTask()
        {
            return PlugModTask.this;
        }
